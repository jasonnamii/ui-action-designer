...
| ⑤ | `she-protocol.md` | 항상 |
| ⑤-b | `@ref: references/ux-principles.md#A` + `#B` | 항상 (Nielsen 10 + Norman 5) |
...
**역할:** SHE 게이트 통과 후, 제안 직전에 UX 원리 전수 점검. 허브 로드: `@ref: references/ux-principles.md#A` + `#B`.

**SHE와의 관계:** SHE = 타이밍·노출 제어 / UX_GATE = 상호작용 품질 제어. SHE 결과를 덮어쓰지 않고 위에 얹는다.

| 검증 축 | 원리 | 체크 |
|--------|------|------|
| 상태 | N1 VISIBILITY · D3 FEEDBACK | 로딩·결과·변화가 보이는가 |
| 언어 | N2 MATCH_REAL | 사용자 언어·은유인가 |
| 자유 | N3 CONTROL | Cancel·Undo·Exit 있는가 |
| 일관 | N4 CONSISTENCY | 토큰·패턴·용어 일관인가 |
| 예방 | N5 ERR_PREV · D5 CONSTRAINTS | 실수 전 차단하는가 |
...
| 시사 | D1 AFFORDANCE · D2 SIGNIFIER | 뭘 할 수 있는지 보이는가 |
| 매핑 | D4 MAPPING | 컨트롤-결과 대응이 직관인가 |

**🚪 게이트:** 10+5=15개 중 12개+ PASS → ⑥ 진행. 11개↓ → SHE L3 재설계 or ④로 롤백. 상세 → 허브 §F 30초 체크리스트.

**SHE 충돌시:** UX 원리가 SHE 숨김 결정과 충돌 (예: N6 RECOGNITION ↔ SHE H 숨김) → SHE 결정 우선 + 그 상태에서 원리 조정안 제시.
